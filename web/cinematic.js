...
export function battleSequence(root,before,after,move,{effects,onEnd,sound=()=>{}}){
 ...
 const genericBurst=async()=>{const glyph={electric:'ϟ',fire:'◆',water:'●',grass:'❧',psychic:'✧',dark:'☾',fighting:'✦',normal:'★',metal:'⬡',dragon:'◇'}[type]||'✦';const tasks=[],count=reduced?3:18;for(let i=0;i<count;i++){const p=node('type-particle',glyph),angle=i/count*Math.PI*2,reach=80+(i%4)*24;p.style.color=TYPES[type].colour;tasks.push(animate(p,[{opacity:0,transform:'translate(0,0) scale(.2)'},{opacity:1,offset:.15},{opacity:0,transform:`translate(${Math.cos(angle)*reach}px,${Math.sin(angle)*reach}px) rotate(${i*35}deg) scale(${i%2?1.4:.5})`}],650+i%3*60).then(()=>p.remove()));}const ring=node('impact-ring');tasks.push(animate(ring,[{opacity:.9,transform:'scale(.12)'},{opacity:0,transform:'scale(2.5)'}],620).then(()=>ring.remove()));await Promise.all(tasks);};
 const typeAttack=async(special)=>{
  const attackAnimate=(el,frames,ms,extra)=>animate(el,frames,special?ms*2.2:ms,extra);
  ...
  if(type==='electric'){
   ...
   for(let i=0;i<(special?14:8);i++){const leaf=document.createElement('i');leaf.className='leaf';leaf.style.left=`${45+(i%6)*6}%`;leaf.style.top=`${48-(i%4)*3}%`;fx.append(leaf);tasks.push(attackAnimate(leaf,[{opacity:0,transform:'translate(-70px,55px) rotate(0deg) scale(.5)'},{opacity:1,offset:.2},{opacity:0,transform:`translate(${35+(i%5)*20}px,${-35-(i%4)*18}px) rotate(${210+i*35}deg) scale(1)`}],650+i*20));}
 ...
 const run=async()=>{
  ...
  if(['basic','special'].includes(move)){
   ...
   if(special){const cut=document.createElement('div');cut.className='special-cutin'+(rare?' ex-cutin':'');const portrait=document.createElement('img');portrait.src=actor.art;portrait.alt='';const label=document.createElement('strong');label.textContent=(rare?'EX · ':'')+actor.name;const skill=document.createElement('small');skill.textContent=actor.power;label.append(skill);cut.append(portrait,label);layer.append(cut);await animate(cut,reduced?[{opacity:0},{opacity:1,offset:.15},{opacity:1,offset:.88},{opacity:0}]:[{opacity:0,transform:'translateX(-115%) skewX(-8deg)'},{opacity:1,transform:'translateX(0) skewX(-8deg)',offset:.15},{opacity:1,transform:'translateX(0) skewX(-8deg)',offset:.88},{opacity:0,transform:'translateX(115%) skewX(-8deg)'}],2400,{easing:'linear'});cut.remove();}
   ...
   lines.remove();banner.remove();await animate(own,[{transform:special?'translate(52%,-68%) scale(1.42)':'translate(45%,-58%) scale(1.34)'},{transform:'translate(0,0) scale(1)',opacity:1}],360);
  }
  if(cancelled)return;
