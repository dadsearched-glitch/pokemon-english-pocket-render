...
const server=http.createServer(async(req,res)=>{
 ...
 try{const pathname=decodeURIComponent(new URL(req.url,'http://pocket-english.internal').pathname);if(['/battle-v3-preview.html','/battle-v3-preview.js'].includes(pathname)){res.writeHead(404);res.end('Not found');return;}let file=path.resolve(root,'.'+pathname);if(file!==root&&!file.startsWith(root+path.sep))throw Error();if(fs.existsSync(file)&&fs.statSync(file).isDirectory())file=path.join(file,'index.html');if(!fs.existsSync(file)){res.writeHead(404);res.end('Not found');return;}res.writeHead(200,{'Content-Type':mime[path.extname(file)]||'application/octet-stream','Cache-Control':'no-store'});fs.createReadStream(file).pipe(res);}catch{res.writeHead(400);res.end('Bad request');}
});
