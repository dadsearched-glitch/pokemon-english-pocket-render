...
async function open(name){
  ...
  await page.getByRole('button',{name:/Boss Raid/}).click();
  return {context,page};
}
const play=async page=>{
  for(let i=0;i<300;i++){
    const state=await page.evaluate(()=>{
      ...
      if(/레이드 성공|아쉽게 막혔|연결이 복구/.test(t))return 'done';
...
try{
  ...
  await host.page.getByRole('button',{name:/Start raid/}).click();
  await host.page.waitForSelector('.mp-arena.raid',{timeout:15000});
  await host.page.screenshot({path:'screenshots/raid-start.png'});
  ...
  assert(h&&g,'raid did not finish');
  const text=await host.page.innerText('#together');
  assert.match(text,/레이드 성공|아쉽게 막혔/);
  console.log('BROWSER_RAID_OK',text.slice(0,260).replace(/\s+/g,' '));
  await host.page.screenshot({path:'screenshots/raid-end.png'});
