...
const rooms=createSignaling();
const server=http.createServer(async(req,res)=>{if(await rooms(req,res))return;if(res.headersSent||res.writableEnded)return;try{let file=path.resolve(root,'.'+decodeURIComponent(new URL(req.url,'http://localhost').pathname));if(!file.startsWith(root+path.sep)&&file!==root){res.writeHead(403);return res.end();}if(fs.existsSync(file)&&fs.statSync(file).isDirectory())file=path.join(file,'index.html');if(!fs.existsSync(file)){res.writeHead(404);return res.end('Not found');}res.writeHead(200,{'Content-Type':mime[path.extname(file)]||'application/octet-stream','Cache-Control':'no-cache'});fs.createReadStream(file).pipe(res);}catch{if(!res.headersSent&&!res.writableEnded){res.writeHead(400);res.end();}}});
server.on('upgrade',(req,socket,head)=>rooms.upgrade(req,socket,head));
server.listen(Number(process.env.PORT||4177),'0.0.0.0',()=>console.log(`Pocket Adventure ready: http://localhost:${process.env.PORT||4177}`));
