...
}).listen(Number(process.env.PORT||4190),'0.0.0.0',()=>{const port=process.env.PORT||4190;const addresses=Object.values(os.networkInterfaces()).flat().filter(a=>a&&a.family==='IPv4'&&!a.internal).map(a=>'http://'+a.address+':'+port);console.log('Together server listening on 0.0.0.0:'+port+(addresses.length?' | LAN '+addresses.join(', '):''));});
